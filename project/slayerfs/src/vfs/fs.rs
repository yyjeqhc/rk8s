    ...
    M: MetaStore + 'static,
{
    ...
    pub async fn closedir_handle(&self, fh: u64) -> Result<(), MetaError> {
        ...
        // Check and abort background prefetch task if still running
        let is_done = handle.prefetch_done.load(Ordering::Acquire);
        if !is_done {
            // Task is still running, need to abort it
            // We can't access prefetch_task directly from Arc, so we use a flag
            // The task will be dropped when the Arc is dropped, which cancels it
            tracing::debug!(
                "Dir handle fh={}, ino={} released while prefetch still running - task will be cancelled on drop",
                fh,
                handle.ino
            );
        }
